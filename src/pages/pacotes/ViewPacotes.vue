<template>
  <div>
    <div class="mb-2"><h2 class="">Pacotes</h2></div>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                <tr v-for="content in contents" :key="content._id">
                  <td>{{ content.name }}</td>
                  <td>{{ content.durationInDays }} dias</td>
                  <td>{{ formatCurrency(content.price) }}</td>

                  <td>
                    <router-link
                      :to="`/editpacote/${content._id}`"
                      type="button"
                    ...
                    <!-- <button
                      ...
                      @click="deleteContent(content._id, content.name)"
                    >
  ...
  methods: {
    async deleteContent(id, name) {
      ...
      if (confirmResult.isConfirmed) {
        ...
        try {
          ...
          const response = await axios.delete(`api/package/deletes/${id}`, {
            headers: {
              token: token,
            },
          });
    ...
    async fetchData() {
      try {
        ...
        const response = await axios.get("/api/package/getall", {
          headers: {
            token: token,
          },
          params: queryParams,
        ...
        if (response && response.data.packages) {
          this.contents = response.data.packages;
          console.log("Usuários", this.contents);
